...
class NewsController extends ComController{ 
	...
	public function index(){
		...
		// 分页数据
		$list = $model->where($map)->order("Indexfirst desc, writetime desc")->limit($p->firstRow.','.$p->listRows)->select(); 
	...
	public function insert(){
		...
		// $data['specialID'] = I('post.specialID', 0, 'intval');
		$data['specialID'] = I('post.specialID', '', 'trim');
		...
		$data['Indexfirst'] = I('post.Indexfirst', 0, 'trim');
		...
		if($strFile){
			...
			$data['attachpath'] = $arrUploadFile[0];
			$data['attachment'] = $arrUploadFile[1];
		}
