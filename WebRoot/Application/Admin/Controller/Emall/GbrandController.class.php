...
// 商品品牌管理类
class GbrandController extends ComController{ 
	...
	public function  index(){
		...
		// 分页数据
		$list = $model->where($map)->order("orders desc, writetime desc, id desc")->limit($p->firstRow.','.$p->listRows)->select(); 
			 // echo $model->getLastSql();  
	...
	public function insert(){
		...
		$data['wwwurl'] = I('post.wwwurl', '', 'trim');
		$data['desc'] = I('post.desc', '', 'trim');
		$data['orders'] = I('post.orders', '', 'intval');
