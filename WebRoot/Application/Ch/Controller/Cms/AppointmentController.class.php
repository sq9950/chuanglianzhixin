...
class AppointmentController extends ComController{ 
	  ...
	  public function  index(){  
		...
		// $count      = $Appointment->where('status=1 and classID=8')->count();// 查询满足要求的总记录数
		// $Page       = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数(25)
		// $show       = $Page->show();
		// // 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		// $list = $Appointment->where('status=1 and classID=8')->order('writetime')->limit($Page->firstRow.','.$Page->listRows)->select();
	 ...
	 public function add(){
		...
		$data['deviceNum'] = $_POST['deviceNum'];
		$data['serverNum'] = $_POST['serverNum'];
		$data['systemNum'] = $_POST['systemNum'];
		$data['virtualNum'] = $_POST['virtualNum'];
